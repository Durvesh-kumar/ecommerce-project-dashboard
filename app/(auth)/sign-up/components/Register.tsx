...
export default function Register({loading, setLoading}:{loading: boolean, setLoading: (value: boolean)=> void}) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if(data.success){
      router.push(`/sign-up/${values.email}`);
    ...
    <div className="w-full mt-10">
      <Form {...form}>
        ...
        <FormField
          ...
          <Button type="submit" className="w-full" disabled={loading}>{loading ? <LoaderCircle size={48} className="w-6 h-6 animate-spin text-white"/> : "Submit"}</Button>
        </form>
      </Form>
    </div>
  );
